...
totalCarouselItems = 5; // total items count
...
function nextCarouselItem() {
    ...
    updateCarouselState(currentCarouselIndex);
}

// Prev item (agar chahiye)
function prevCarouselItem() {
    if (currentCarouselIndex <= 0) {
        currentCarouselIndex = totalCarouselItems - 1;
    } else {
        currentCarouselIndex--;
    }
    updateCarouselState(currentCarouselIndex);
...
function updateCarouselState(newIndex) {
    ...
    if (container) {
        ...
        if (window.innerWidth >= 1024) {
            // --- yaha fix kiya ---
            translateX = -(newIndex * (100 / 3)); // har slide 33.33% width ki hai
